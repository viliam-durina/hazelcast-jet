...
public class CustomSessionWindowTest {
    ...
    @Before
    public void before() {
        supplier = () -> lastSuppliedProcessor = new CustomWindowP<>(
                singletonList((DistributedToLongFunction<Entry<Object, Long>>) Entry::getValue),
                singletonList(entryKey()),
                AggregateOperations.counting(),
                MergingWindowSet::new,
                (item, ts) -> singletonList(new CustomWindowP.WindowDef(ts, ts + SESSION_TIMEOUT)),
                new Trigger<Entry<String, Long>, Object>() { },
                WindowResult::new,
    ...
    public void after() {
        ...
        assertTrue("windowSets not empty: " + lastSuppliedProcessor.windowSets,
                lastSuppliedProcessor.windowSets.isEmpty());
        assertTrue("eventTimers not empty: " + lastSuppliedProcessor.eventTimers,
                lastSuppliedProcessor.eventTimers.isEmpty());
        assertTrue("systemTimers not empty: " + lastSuppliedProcessor.systemTimers,
                lastSuppliedProcessor.systemTimers.isEmpty());
    }
...
//        events.addAll(eventsWithKey("c"));
    ...
    private void assertCorrectness(List<Object> events) {
        List<Object> expectedOutput = events.stream()
                                            ...
                                            .flatMap(CustomSessionWindowTest::expectedSessions)
                                            .distinct()
                                            .collect(toList());
    ...
    public static void main(String[] args) throws Exception {
        for (int i = 0; i < 10; i++) {
            CustomSessionWindowTest test = new CustomSessionWindowTest();
            test.before();
            test.runBench();
        }
    }
