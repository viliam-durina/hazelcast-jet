...
final class JoinScanProcessorSupplier implements ProcessorSupplier, DataSerializable {
    ...
    public Collection<? extends Processor> get(int count) {
        ...
        for (int i = 0; i < count; i++) {
            Processor processor =
                    new TransformBatchedP<JetSqlRow, JetSqlRow>(joinFn(joinInfo, map, rightRowProjectorSupplier, context)) {
                        @Override
    ...
    private static FunctionEx<Iterable<JetSqlRow>, Traverser<JetSqlRow>> joinFn(
        ...
        Projection<Entry<Object, Object>, JetSqlRow> projection = QueryUtil.toProjection(rightRowProjectorSupplier);
    ...
    private static boolean join(
        ...
        for (JetSqlRow right : rights) {
            JetSqlRow joined = ExpressionUtil.join(left, right, condition, context);
            if (joined != null) {
                rows.add(joined);
                matched = true;
            }
